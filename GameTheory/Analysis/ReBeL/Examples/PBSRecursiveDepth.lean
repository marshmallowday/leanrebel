...
def recursiveInitialProfile (tolerance : ℝ) : Profile (model fullPrior).behavioralSignature :=
  pbsRecursiveDepth pbsRecursiveAllocatedNoise [1, 1, 1] (reducedModel fullPrior)
...
theorem recursiveInitial_empty (tolerance : ℝ) :
    pbsRecursiveDepth pbsRecursiveAllocatedNoise [] (reducedModel fullPrior)
      pbsRootControlFallback cfrPayoff 2 recursiveInitialBelief tolerance =
...
theorem recursiveInitial_zero_cut (tolerance : ℝ) (positive : 0 < tolerance) :
    IsNash (behavioralBeliefForm (model fullPrior) recursiveInitialBelief 3)
      (euPreferenceWithin tolerance (fun h who => cfrPayoff who h))
      (pbsRecursiveDepth pbsRecursiveAllocatedNoise [0, 1, 1, 1] (reducedModel fullPrior)
        pbsRootControlFallback cfrPayoff 2 recursiveInitialBelief tolerance) := by
...
theorem recursiveInitial_noise_positive (tolerance : ℝ) (positive : 0 < tolerance)
    ...
    0 < pbsRecursiveAllocatedNoise (reducedModel fullPrior) recursiveInitialBelief.law
      (pbsDepthAllocationError (pbsRootDepthErrorFactor (reducedModel fullPrior)
        recursiveInitialBelief.law pbsRootControlFallback 1 2) tolerance) n trunk who info :=
...
theorem recursiveChild_leafOptimal (loss : ℝ) (positive : 0 < loss) (who : Player) :
    CFRDLeafOptimal (model fullPrior)
      (cfrDComposedChildContinuation (reducedModel fullPrior) (carriedBitProfile false)
        ...
        (pbsRecursiveDepth pbsRecursiveAllocatedNoise [1] (reducedModel fullPrior)
          pbsRootControlFallback cfrPayoff 2) (fun h player => cfrPayoff player h))
